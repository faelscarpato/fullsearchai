...
const Infographic: React.FC<InfographicProps> = ({ image, onEdit, isEditing }) => {
  ...
  return (
    <div className="flex flex-col items-center w-full max-w-6xl mx-auto animate-in fade-in zoom-in duration-700 mt-8">
      ...
      {/* Image Container */}
      <div className="relative group w-full bg-zinc-50 dark:bg-black rounded-2xl overflow-hidden shadow-2xl border border-zinc-200 dark:border-zinc-800">
        {/* Decorative Corner Markers */}
        <div className="absolute top-0 left-0 w-16 h-16 border-t-2 border-l-2 border-amber-500/30 rounded-tl-2xl z-20 pointer-events-none"></div>
        <div className="absolute top-0 right-0 w-16 h-16 border-t-2 border-r-2 border-amber-500/30 rounded-tr-2xl z-20 pointer-events-none"></div>
        <div className="absolute bottom-0 left-0 w-16 h-16 border-b-2 border-l-2 border-amber-500/30 rounded-bl-2xl z-20 pointer-events-none"></div>
        <div className="absolute bottom-0 right-0 w-16 h-16 border-b-2 border-r-2 border-amber-500/30 rounded-br-2xl z-20 pointer-events-none"></div>
        ...
        <div className="absolute top-6 right-6 flex gap-2 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity z-30">
          <button
            onClick={() => setIsFullscreen(true)}
            className="bg-black/60 backdrop-blur-md text-white p-3 rounded-xl shadow-lg hover:bg-amber-600 transition-colors border border-white/10 block"
      ...
      <div className="w-full max-w-3xl -mt-6 sm:-mt-8 relative z-40 px-4">
        <div className="bg-white/90 dark:bg-zinc-900/80 backdrop-blur-xl p-3 sm:p-2 sm:pr-3 rounded-2xl shadow-2xl border border-zinc-200 dark:border-white/10 flex flex-col sm:flex-row gap-2 items-center ring-1 ring-black/5 dark:ring-white/5">
          <div className="pl-4 text-amber-600 dark:text-amber-400 hidden sm:block">
          ...
          <form onSubmit={handleSubmit} className="flex-1 w-full flex flex-col sm:flex-row gap-2">
            <input
              ...
              className="flex-1 bg-zinc-50 dark:bg-black sm:bg-transparent border border-zinc-200 dark:border-white/5 sm:border-none rounded-xl sm:rounded-none focus:ring-0 text-zinc-900 dark:text-white placeholder:text-zinc-400 px-4 py-3 sm:px-2 sm:py-2 font-medium text-base"
              disabled={isEditing}
            />
      ...
      <div className="mt-8 text-center space-y-2 px-4">
        <p className="text-xs text-zinc-500 dark:text-zinc-500 font-mono max-w-xl mx-auto truncate opacity-60">
          PROMPT: {image.prompt}
        </p>
      </div>
